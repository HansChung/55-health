...
export function ChapterOpeningScreen({ chapter }: ChapterOpeningScreenProps) {
  ...
  const [picked, setPicked] = useState<ChapterEntryId | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      const raw = localStorage.getItem(PICKED_ENTRY_KEY);
      if (raw) {
        const parsed = JSON.parse(raw) as { id?: ChapterEntryId; note?: string };
  ...
  const savePick = (id: ChapterEntryId, note?: string) => {
    setPicked(id);
    ...
    <>
      ...
      <div className="no-print" style={{
        ...
        <SubPage
          ...
          onBack={() => router.push("/")}
          accent="linear-gradient(180deg, #E8F4FA 0%, transparent 55%)"
          footer={
            <button
        ...
        >
          ...
          <textarea
            ...
            placeholder="例如：我比較想先試「問一句」，因為最近有很多想問的事…"
            rows={3}
            style={{
      ...
      <div id="chapter-print-card" style={{
        ...
        <PrintRouteCard chapter={chapter} picked={picked} reflectNote={reflectNote} />
      </div>
    </>
  );
}
...
function PrintRouteCard({
  chapter,
  picked,
  reflectNote,
}: {
  chapter: ChapterOpening;
  picked: ChapterEntryId | null;
  reflectNote: string;
}) {
  return (
    <div style={{ maxWidth: 480, margin: "0 auto" }}>
      ...
      <p style={{ fontSize: 14, margin: "0 0 20px" }}>{chapter.tryPrompt}</p>
      ...
      <p style={{ fontSize: 12, color: "#666" }}>
        掃碼網址：{typeof window !== "undefined" ? window.location.origin : ""}/smart/chapter/{chapter.id}
      </p>
    </div>
  );
}

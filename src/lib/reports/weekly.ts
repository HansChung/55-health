// ────────────────────────────────────────────────
// 每週健康報告計算 — 純資料運算，給 API route 與 cron 共用
// 傳入任一 Supabase client（user-scoped 或 admin）+ userId
// ────────────────────────────────────────────────
...
export interface WeeklyReport {
  ...
  exercise: { minutes: number; kcal_burned: number };
  health: {
...
/** 計算某位用戶過去 7 天的週報。回傳 null 代表查詢失敗。 */
export async function computeWeeklyReport(
  ...
  if (mealsRes.error || exercisesRes.error || metricsRes.error) {
    console.error("[weekly-report] 讀取失敗:", mealsRes.error || exercisesRes.error || metricsRes.error);
    return null;
  }
  ...
  const days = new Set(
    meals.map((m) =>
      new Date(new Date(m.eaten_at).getTime() + 8 * 3600 * 1000).toISOString().substring(0, 10)
    )
  );
  ...
  return {
    ...
    exercise: { minutes: exerciseMinutes, kcal_burned: exerciseCalories },
    health: {
      weight: latestWeight
        ? {
            latest: latestWeight.weight_kg,
            change:
              previousWeight?.weight_kg && latestWeight.weight_kg
      ...
      blood_glucose: latestBloodGlucose
        ? { value: latestBloodGlucose.glucose_mg_dl, status: getGlucoseStatus(latestBloodGlucose) }
        : null,
    },
    tips,
...
function buildTips(input: {
  ...
  if (tips.length < 3 && input.avgCalories > 0)
    tips.push(`這週平均每天約 ${input.avgCalories} 大卡，可以對照您的每日目標調整。`);

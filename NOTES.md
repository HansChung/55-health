...
> 最後更新：2026-05-26
> 上一輪由 Cursor 加了：用藥提醒、合作活動、語音控費、每週報告、方案分級、LINE 分享、提醒中心、後台 CTR
...
| Resend（SMTP）| resend.com | `onboarding@resend.dev`（test domain，只能寄給自己）|
...
├── screens/                                   # 20 個畫面
│   ├── home-screen.tsx                        # 首頁（含真實 macro、用藥提醒、合作活動 CTA）
...
### 高優先級
1. **Google OAuth 還是「Testing」模式** — 朋友的 Gmail 要先加到測試清單才能登入
...
2. **AI 建議改完餐點後不會自動更新** — 要等 1 小時快取過期或用戶手動「換個建議」
   - **建議改法**：`handleSaveMeal` 完成後呼叫 `loadSuggestion(true)`

### 中優先級
3. **OpenAI Realtime 對話內容沒存 DB** — 雖然 `conversations` 表有，但語音對話沒寫入
4. **管理後台「對話記錄」是佔位** — 沒實作（已有 conversations 表）
5. **每週報告是手動觸發**（用戶開頁面才生成）— 沒有 Cron 自動寄 email
6. **PWA / 通知推播沒做** — 純 web 沒辦法做手機背景提醒（要 Capacitor 打包才行）
...
### 立即（小修小補）
1. **AI 建議改完餐點後自動更新**
   - `handleSaveMeal` 成功後加 `loadSuggestion(true)`
2. **語音對話內容存 DB**
   - `RealtimeClient` 的 `onUserTranscript` / `onAssistantTranscript` 寫入 `conversations` 表
3. **管理後台「對話記錄」實作**
   - 從 `conversations` 表抓資料 + 分頁

### 短期（差異化）
4. **每週報告自動寄 email**
   - Vercel Cron（`vercel.json`）每週日早上 8 點
   - 呼叫 `/api/reports/weekly` 生成
   - 用 Resend 寄給用戶 + 家人（讀 `family_links`）
5. **緊急 SOS 按鈕**
   - 首頁角落小按鈕，長按 3 秒
   - 一鍵 `tel:` 撥電話給家人 + 傳 LINE
6. **連續打卡 + 健康成就徽章**
   - DB 加 `user_streaks` 表
   - 連續 7 天記錄 → 跳通知
...
8. **Google OAuth 提交審核** — 消除「未驗證」警告（需要自訂網域）
9. **買網域 + 接 Vercel + 設 Resend 自訂 sender** — 解決 Email 寄信限制

...
> 早期幾輪加了：用藥提醒、合作活動、語音控費、每週報告、方案分級、LINE 分享、提醒中心、後台 CTR、語音對話存 DB、管理後台對話記錄、刪餐後刷新建議、健康成就徽章、緊急 SOS 按鈕、成就首頁入口 + 解鎖慶祝動畫
> 最近修的 bug：語音 session_id 無 crypto.randomUUID 時用合法 UUID v4 fallback、成就連續天數改用固定 UTC+8 計算（修正 Vercel UTC 伺服器跨日問題）

---

## 🆕 2026-06 品質強化輪（P0/P1/P2 + PWA/CI/SEO/週報）

這一輪沒加新「功能」，而是把**穩定性、安全、無障礙、體驗、可維護性、可被發現性**補強。重點如下（皆已上 main 並由 Vercel 部署）：

### P0（對長者體驗 / 資料正確性最關鍵）
- **Error Boundary 防白屏**：`src/app/error.tsx`、`src/app/global-error.tsx`，render 例外不再白屏，改顯示「再試一次」友善畫面。
- **全域 Toast 取代 24 處原生 `alert()`**：`src/hooks/use-toast.tsx`（`ToastProvider` 掛在 layout）。錯誤訊息全面人話化，技術細節只進 console。**新功能要提示用戶時用 `useToast()`，不要用 `alert()`。**
- **網路韌性**：`src/lib/api-client.ts` 的 `apiFetch` 加 15 秒 timeout + GET 自動重試 2 次（指數退避，POST/PATCH/DELETE 不重試避免重複寫入）；新增 `ApiError` class（含 `isNetwork`）。
- **樂觀更新真 rollback**：`handleSaveMeal` 失敗時還原快照，不再靠重抓 → 修「以為記錄成功其實沒存」風險。
- **離線偵測橫幅**：`src/components/offline-banner.tsx`（掛在 layout），斷線時最上方提示。
- **無障礙**：補齊 aria-label（鈴鐺、相機 FAB、各關閉/返回/刪除鈕、語音鈕）；`globals.css` 加全域 `:focus-visible` 焦點樣式。
- **安全**：`alerts/[id]` PATCH 加擁有權驗證（本人或 accepted+alerts 權限家人），不再純靠 RLS。

### P1（商業化前防護）
- **錯誤訊息收斂**：全站約 45 處 API 不再回傳 `error.message`/zod detail/上游錯誤文字，改 `console.error` + 通用訊息（防 DB 結構外洩）。**新 route 請沿用此原則。**
- **合作活動 track 端點改為需登入**（`partner-campaigns/[id]/track`），堵匿名灌數據。
- **語音計費完整性**：建立 session 時記 server 端 `started_at` + `max_seconds`（`ai_usage` 一筆 pending 列）；回報用量時取 client 回報與 server 經過秒數的較大值並 clamp，防低報繞配額；同一 session 更新同一筆，不重複計。
- **Stripe 退訂改用 `stripe_customer_id` 對應**（email+1000 上限改為後備）；checkout body 加解析防護。**需先跑 `supabase/add-stripe-customer.sql`（啟用 Stripe 前再跑即可）。**
- **自動化測試（vitest）**：`npm test` / `npm run test:watch`，30 項覆蓋 medication-utils、achievements streak、AI 計費、feature-gates。

### P2（架構債）
- 抽出 `src/hooks/use-display-settings.tsx`（字級/對比邏輯移出 page.tsx）。
- page.tsx 衍生值改用 `useMemo`（totalCal / medicationReminders / allHealthAlerts / healthAlerts / yesterdayMealsByType）。
- 抽出 `src/components/nav-card.tsx`（首頁重複入口卡片共用）。
- 餐點縮圖加 `loading="lazy"` + `decoding="async"`。

### PWA / CI / SEO / 週報
- **PWA**：`app/icon.svg`、`app/apple-icon.svg`、`app/manifest.ts`，可「加到主畫面」當 App 用（免 Capacitor）。
- **CI**：`.github/workflows/ci.yml`，push/PR 自動跑 tsc + test + build。
...
↓
Vitest（單元測試）+ GitHub Actions（CI：push 自動跑 tsc+test+build）
PWA（manifest + icon，可加到主畫面）
```
...
- [x] Vercel `NEXT_PUBLIC_APP_URL` = `https://nuan55.com` 並 Redeploy
- [x] Supabase Site URL + Redirect URLs 已指向 nuan55.com
- [x] Google OAuth 同意畫面已填 nuan55.com（Publish / 送審）
- [x] Resend **nuan55.com** 網域已驗證
- [x] 程式／Email 模板已以 `nuan55.com` 為對外網址
- [x] 登入 `redirectTo` 用 `window.location.origin` → 在 nuan55.com 上自動走正確 callback
- [x] Smoke test（2026-05-31）：首頁 /privacy /terms /pricing 皆 HTTP 200
...
### 高優先級
1. ~~**Google OAuth Testing 模式**~~ → 已 Publish / 設定 nuan55.com（若仍見「未驗證」可等 Google 審核或讓用戶點「進階→繼續」）

### 中優先級
2. ~~**通知推播（背景提醒）沒做**~~ → **Web Push 已接**（異常預警）；用餐／喝水定時提醒仍偏原生 App。需 VAPID 環境變數 + `add-push-subscriptions.sql`
...
- ~~PWA 沒做~~ → **已 PWA 化**（manifest + icon，可加到主畫面）
...
- ~~語音 session_id 無 crypto.randomUUID 時存檔失敗~~ → 改用合法 UUID v4 fallback
- ~~成就連續天數用 UTC 算被歸前一天~~ → 改固定 UTC+8 計算
- ~~render 例外白屏 / 大量 alert / 網路掛起卡死 / API 錯誤外洩 / alerts 越權~~ → 見上方「2026-06 品質強化輪」

### 低優先級
7. **Stripe 訂閱付費未啟用** — 程式寫好了，需要 4 個 key + 跑 `supabase/add-stripe-customer.sql`
8. **Android 還沒實測** — Capacitor 設定好但沒在真機跑過
9. ~~**Resend 網域驗證**~~ → nuan55.com 已驗證，`noreply@nuan55.com` 可對外寄信
...
# 3. push 前可先驗證（CI 也會自動跑這些）
npm test            # vitest 單元測試（30 項）
npx tsc --noEmit    # 型別檢查
...
# 5. ☕ 等 1-2 分鐘
# - GitHub Actions（.github/workflows/ci.yml）自動跑 tsc + test + build
# - Vercel 自動部署到 https://nuan55.com（舊 vercel.app 網址仍可用）
```

**git remote 走 SSH（`git@github.com:HansChung/55-health.git`）。**
**不要手動跑 `npm run build` 當作部署，直接 `git push` 讓 Vercel 跑就好。**
